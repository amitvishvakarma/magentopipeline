...
define(
    [
        ...
        'Ced_ConnectBase/js/modal/popup',
        'vkbeautify',
        'highlight',
        'j2t'
        ...
        return Column.extend(
            {
                defaults: {
                    bodyTmpl: 'Ced_ConnectBase/ui/grid/cells/actions',
                    sortable: false,
                    draggable: false,
                    actions: [],
                    rows: [],
                ...
                feedAjax: function (url, feedId, columnName) {
                    let message = null;
                    $.ajax(
                        {
                            url: url,
                ...
                _popup: function (action, callback) {
                    action.popup.message = this.feedAjax(action.popup.ajax_url, action.popup.feed_id, action.popup.name);
                    ...
                    if (download === false) {
                        var content = '<textarea id="raw_message" style="position: absolute; left: -999em;" aria-hidden="true"></textarea><div style="max-height: 450px; overflow: auto;"><pre><code>' + data.message + '</code></pre></div>';
                        if (data.render === 'html' && dataType === 'json') {
                            ...
                            content = '<textarea id="raw_message" style="position: absolute; left: -999em;" aria-hidden="true"></textarea><div style="max-height: 450px; overflow: auto;">' + result + '</div>';
                        }

                        if (data.shop_copy_clipboard !== undefined && data.shop_copy_clipboard === true && data.message) {
                            popup(
                                {
                                    title: data.title,
                                    content: content,
